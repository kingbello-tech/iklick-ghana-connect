...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!user_id || !password || typeof password !== "string" || password.length < 6) {
      return new Response(JSON.stringify({ error: "user_id and a password of at least 6 characters are required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
